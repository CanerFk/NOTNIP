    ...
    return (
        ...
        >
            <div className="search-bar-row">
                <div className="search-input-wrapper">
                    ...
                    <span className="search-counter">
                        {searchTerm ? `${matchInfo.count > 0 ? matchInfo.activeIndex + 1 : 0}/${matchInfo.count}` : ''}
                    </span>
                </div>
                <div className="search-actions">
                    <button onClick={() => editor.commands.goToPrevMatch()} className="search-btn" title="Previous (Shift+Enter)" disabled={matchInfo.count === 0}>
                        <ChevronUp size={14} />
                    </button>
                    <button onClick={() => editor.commands.goToNextMatch()} className="search-btn" title="Next (Enter)" disabled={matchInfo.count === 0}>
                        <ChevronDown size={14} />
                    </button>
                    <button onClick={handleClose} className="search-btn search-close-btn ml-1" title="Close (Escape)">
                        <X size={14} />
                    </button>
                </div>
            </div>
            ...
            <div className="search-bar-row">
                ...
                <div className="search-actions">
                    <button onClick={handleReplace} className="search-btn search-btn-text replace-hover" title="Replace (Enter)" disabled={matchInfo.count === 0}>
                        Rep
                    </button>
                    <button onClick={handleReplaceAll} className="search-btn search-btn-text replace-hover" title="Replace All (Shift+Enter)" disabled={matchInfo.count === 0}>
                        All
                    </button>
                </div>
            </div>
