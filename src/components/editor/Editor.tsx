              ...
              if (dispatch) {
                ...
                tr.setSelection(
                  (state.selection.constructor as any).near(
                    tr.doc.resolve(doc.content.size - 1),
                  ),
                );
              }
              return true;
      ...
      "Mod-a": ({ editor }) => {
        ...
        for (let depth = $from.depth; depth >= 1; depth--) {
          ...
          const alreadySelected =
            selection.from === startOfBlock && selection.to === endOfBlock;
...
const STYLE_TITLE_INPUT = {
  color: "var(--editor-title-color, var(--accent))",
  lineHeight: "1.15",
};
...
export function Editor({
  ...
  const readableLineLength = useStore(
    (state) => state.themePreferences.readableLineLength,
  );
  ...
  const exportSuccessTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const editor = useEditor(
    {
            ...
            return {
              ...
              listType: {
                ...
                parseHTML: (element: HTMLElement) =>
                  element.getAttribute("data-list-type") || "disc",
          ...
          addAttributes() {
            return {
              ...
              listType: {
                default: "decimal",
                parseHTML: (element: HTMLElement) =>
                  element.getAttribute("data-list-type") || "decimal",
                renderHTML: (attributes: any) => {
          ...
          addAttributes() {
            return {
              ...
              hasCaption: {
                default: false,
                parseHTML: (element) =>
                  element.hasAttribute("data-has-caption"),
                renderHTML: (attributes) => {
          ...
          addAttributes() {
            return {
              ...
              striped: {
                default: true,
                parseHTML: (element) =>
                  element.getAttribute("data-striped") !== "false",
                renderHTML: (attributes) => {
      ...
      editorProps: {
        attributes: {
          class:
            "prose mx-auto focus:outline-none max-w-none pb-32 min-h-screen",
        },
              ...
              if (file.size > MAX_SIZE) {
                ...
                if (wantLocal) {
                  ...
                  if (selectedPath) {
                    ...
                    const node = isImage
                      ? schema.nodes.resizableImage.create({
                          src,
                          alt: file.name,
                        })
  ...
  return (
    <div className="flex flex-col h-full w-full bg-background relative overflow-hidden transition-colors duration-300">
      {breadcrumbs.length > 1 && (
        <div
          className="flex items-center gap-0 px-6 sm:px-8 py-1.5 bg-secondary/50 border-b border-border select-none flex-shrink-0 overflow-x-auto custom-scrollbar"
          style={STYLE_SCROLLBAR_NONE}
        >
          {breadcrumbs.map((crumb, idx) => {
            ...
            return (
              <div key={crumb.id} className="flex items-center gap-0 min-w-0">
                <button
                  onClick={() => {
                    if (!isLast) setActivePage(crumb.id);
                  }}
                  className={`flex items-center gap-1.5 px-2 py-0.5 font-mono text-[11px] tracking-wide transition-colors whitespace-nowrap ${
                    isLast
                ...
                >
                  <span className="flex-shrink-0 opacity-70">
                    {renderIcon(crumb.icon || "text", 12)}
                  </span>
                  <span className="truncate max-w-[120px]">{crumb.title}</span>
                </button>
                {!isLast && (
                  <span className="text-muted/40 text-[10px] mx-0.5 font-mono select-none">
                    /
                  </span>
                )}
              </div>
      ...
      >
        <div
          className={cn(
            ...
            readableLineLength
              ? "max-w-4xl px-6 sm:px-8"
              : "max-w-none px-6 sm:px-12 md:px-24 xl:px-48",
          )}
        >
          <div
            className="flex items-center mb-4 group gap-2"
          ...
          >
            ...
            <div className="relative flex-1">
              <input
                ...
                className="w-full bg-transparent text-4xl font-bold outline-none border-none placeholder-transparent z-10 relative leading-none"
                style={STYLE_TITLE_INPUT}
              ...
              {title === "" && (
                <label
                  htmlFor="note-title"
                  className="absolute left-0 top-0 text-4xl font-bold text-muted/30 pointer-events-none leading-none"
...
function GlobalDeleteHandler() {
  ...
  useEffect(() => {
    if (deletionCandidateId && !page) {
      setDeletionCandidateId(null);
    }
  }, [deletionCandidateId, page, setDeletionCandidateId]);
