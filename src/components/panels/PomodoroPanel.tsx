...
function clampMinutes(val: number): number {
  return Math.min(120, Math.max(1, val));
}
...
export function PomodoroPanel() {
  ...
  const totalDuration = getDurationForMode(
    currentMode,
    focusDuration,
    shortBreakDuration,
    longBreakDuration,
  ...
  const [workInput, setWorkInput] = useState(String(Math.round(wDur / 60)));
  const [shortInput, setShortInput] = useState(String(Math.round(sbDur / 60)));
  const [longInput, setLongInput] = useState(String(Math.round(lbDur / 60)));

  useEffect(() => {
    setWorkInput(String(Math.round(wDur / 60)));
  }, [wDur]);

  useEffect(() => {
    setShortInput(String(Math.round(sbDur / 60)));
  }, [sbDur]);

  useEffect(() => {
    setLongInput(String(Math.round(lbDur / 60)));
  }, [lbDur]);
  ...
  const toggle = () => {
    if (isRunning) {
      ...
      const remaining =
        pausedRemaining !== null ? pausedRemaining : totalDuration;
      const now = Date.now();
      updatePomodoro({
        isRunning: true,
  ...
  const commitInput = (
    rawVal: string,
    modeKey: "work" | "shortBreak" | "longBreak",
  ) => {
    const parsed = parseInt(rawVal, 10);
    if (!isNaN(parsed)) {
      const clamped = clampMinutes(parsed);
      applyConfiguration(modeKey, clamped * 60);
    }
  };
  ...
  return (
    <div
      className="relative flex flex-col h-full bg-[var(--bg-primary)] overflow-hidden"
    ...
    >
      <div className="flex flex-col h-full z-0 p-3 pb-2">
        <div className="flex items-center justify-between opacity-90 select-none pb-3 border-b-2 border-border mb-2">
          <div
            className={cn(
              "flex items-center gap-2 drop-shadow-sm transition-colors duration-300",
              COLOR_CLASSES[currentMode as keyof typeof COLOR_CLASSES],
            )}
          >
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div
            className={cn(
              "text-[3.5rem] leading-[1] font-bold tracking-[0.02em] drop-shadow-[2px_2px_0_var(--shadow-color)] select-none transition-colors duration-500",
              isRunning ? "text-gruv-green" : "text-main",
            )}
          >
        ...
        <div className="pt-2 flex gap-2 pb-1">
          {skipConfirm ? (
            <div className="flex-1 h-11 flex items-center justify-center gap-2 bg-element border-2 border-border shadow-[3px_3px_0_0_var(--shadow-color)]">
              <span className="text-xs tracking-widest text-muted uppercase">
                SKIP?
              </span>
              <button
                onClick={confirmSkip}
                className="px-2 py-0.5 border-2 border-gruv-green text-gruv-green text-xs font-bold hover:bg-gruv-green hover:text-bg-primary transition-colors"
              >
                YES
              </button>
              <button
                onClick={() => setSkipConfirm(false)}
                className="px-2 py-0.5 border-2 border-gruv-red text-gruv-red text-xs font-bold hover:bg-gruv-red hover:text-bg-primary transition-colors"
              >
                NO
              </button>
            </div>
          ...
          <button
            ...
            className="w-11 h-11 flex-shrink-0 flex items-center justify-center bg-element border-2 border-border hover:bg-gruv-orange hover:border-gruv-orange hover:text-bg-primary transition-all shadow-[3px_3px_0_0_var(--shadow-color)] active:translate-y-[3px] active:translate-x-[3px] active:shadow-none"
            title="Skip Session"
          >
        ...
        <div className="flex-1 flex flex-col gap-2.5 overflow-y-auto custom-scrollbar pr-1">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    onBlur={(e) => commitInput(e.target.value, "shortBreak")}
                    onKeyDown={(e) => {
                      if (e.key === "Enter")
                        commitInput(
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    onBlur={(e) => commitInput(e.target.value, "longBreak")}
                    onKeyDown={(e) => {
                      if (e.key === "Enter")
                        commitInput(
                          (e.target as HTMLInputElement).value,
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-1.5">
              <button
                onClick={() => forceMode("work")}
                className="flex-1 flex items-center justify-center gap-1 py-1.5 px-2 bg-element border-2 border-border hover:bg-gruv-red hover:text-bg-primary transition-colors text-xs"
              >
                <Briefcase size={12} /> Focus
              </button>
              <button
                onClick={() => forceMode("shortBreak")}
                className="flex-1 flex items-center justify-center gap-1 py-1.5 px-2 bg-element border-2 border-border hover:bg-gruv-blue hover:text-bg-primary transition-colors text-xs"
              >
